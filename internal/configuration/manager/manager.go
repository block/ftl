...
func (m *Manager[R]) getData(ctx context.Context, ref configuration.Ref) ([]byte, error) {
	...
	if m.provider.Key() != ProviderKeyForAccessor(key) {
		return nil, errors.Errorf("no provider for scheme %q", key.Scheme)
	}
	var data []byte
	...
	case configuration.AsynchronousProvider[R]:
		...
		if err != nil {
			return nil, errors.Wrapf(err, "%s", ref)
		}
	...
	default:
		if err != nil {
			return nil, errors.Errorf("provider for %s does not support on demand access or syncing", ref)
		}
	}
	return data, nil
}
...
func (m *Manager[R]) Set(ctx context.Context, ref configuration.Ref, value any) error {
	...
	return errors.WithStack(m.SetJSON(ctx, ref, data))
}
...
func (m *Manager[R]) SetJSON(ctx context.Context, ref configuration.Ref, value json.RawMessage) error {
	if err := checkJSON(value); err != nil {
		return errors.Wrapf(err, "invalid value for %s, must be JSON", m.router.Role())
	}
	...
	return errors.WithStack(m.router.Set(ctx, ref, key))
}
...
func (m *Manager[R]) Unset(ctx context.Context, ref configuration.Ref) error {
	...
	return errors.WithStack(m.router.Unset(ctx, ref))
}

func (m *Manager[R]) List(ctx context.Context) ([]configuration.Entry, error) {
	return errors.WithStack2(m.router.List(ctx))
}
...
func checkJSON(data []byte) error {
	...
	var v any
	return errors.WithStack(dec.Decode(&v))
}
