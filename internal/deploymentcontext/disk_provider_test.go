...
func TestDiskProvider(t *testing.T) {
	...
	t.Run("test providers", func(t *testing.T) {
		...
		var actualSecretValue string
		err = json.Unmarshal(secretsMap["test-secret"], &actualSecretValue)
		assert.NoError(t, err)
		assert.Equal(t, "secret-value", actualSecretValue)

		configProvider := NewDiskConfigProvider(configsDir)
		configsMap := configProvider(ctx)
		var actualConfigValue string
		err = json.Unmarshal(configsMap["test-config"], &actualConfigValue)
		assert.NoError(t, err)
		assert.Equal(t, "config-value", actualConfigValue)

		routeProvider, err := NewDiskRouteProvider(ctx, schemaPath)
		assert.NoError(t, err)
		route := routeProvider.Route("testmodule")
		assert.Equal(t, "GET /hello/{name}", route)
	})
}
