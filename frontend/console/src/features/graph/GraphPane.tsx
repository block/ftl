...
export const GraphPane: React.FC<GraphPaneProps> = ({ onTapped }) => {
  ...
  useEffect(() => {
    ...
    if (hasNewNodesWithoutPositions) {
      /**
       * TODO The multiple layout options here are for demo purposes. A single layout should be
       * selected and the unused ones removed.
       */
      const colaLayoutOptions = {
        ...
        convergenceThreshold: 0.01, // when the alpha value (system energy) falls below this value,
        // the layout stops
        nodeSpacing: () => 10, // extra spacing around nodes
        flow: undefined, // use DAG/tree flow layout if specified, e.g. { axis: 'y', minSeparation:
        // 30 }
        alignment: undefined, // relative alignment constraints on nodes, e.g. {vertical: [[{node:
        // node1, offset: 0}, {node: node2, offset: 5}]], horizontal:
        // [[{node: node3}, {node: node4}], [{node: node5}, {node: node6}]]}
        gapInequalities: undefined, // list of inequality constraints for the gap between the
        // nodes, e.g. [{"axis":"y", "left":node1, "right":node2,
        // "gap":25}]
        centerGraph: true, // adjusts the node positions initially to center the graph (pass false
        // if you want to start the layout from the current position)
        ...
        allConstIter: undefined, // initial layout iterations with all constraints including
        // non-overlap
      } as LayoutOptions
      ...
      const dagreLayoutOptions = {
        ...
        align: undefined, // alignment for rank nodes. Can be 'UL', 'UR', 'DL', or 'DR', where U =
        // up, D = down, L = left, and R = right
        acyclicer: undefined, // If set to 'greedy', uses a greedy heuristic for finding a feedback
        // arc set for a graph. A feedback arc set is a set of edges that can
        // be removed to make a graph acyclic.
        ...
        edgeWeight: () => 1, // higher weight edges are generally made shorter and straighter than
        // lower weight edges

        // general layout options
        fit: true, // whether to fit to viewport
        padding: 60, // fit padding
        spacingFactor: 1.5, // Applies a multiplicative factor (>0) to expand or compress the
        // overall area that the nodes take up
        nodeDimensionsIncludeLabels: false, // whether labels should be included in determining the
        // space used by a node
        animate: true, // whether to transition the node positions
        animateFilter: () => true, // whether to animate specific nodes when animation is on;
        // non-animated nodes immediately go to their final positions
        animationDuration: 500, // duration of animation in ms if enabled
        animationEasing: undefined, // easing of animation if enabled
        boundingBox: undefined, // constrain layout bounds; { x1, y1, x2, y2 } or { x1, y1, w, h }
        transform: (_node: NodeSingular, pos: cytoscape.Position) => pos, // a function that
        // applies a transform to
        // the final node
        // position
        ready: () => {}, // on layoutready
        sort: undefined, // a sorting function to order the nodes and edges; e.g. function(a, b){
        // return a.data('weight') - b.data('weight') } because cytoscape dagre
        // creates a directed graph, and directed graphs use the node order as a
        // tie breaker when defining the topology of a graph, this sort function
        // can help ensure the correct order of the nodes/edges. this feature is
        // most useful when adding and removing the same nodes and edges multiple
        // times in a graph.
        stop: () => {}, // on layoutstop
      } as LayoutOptions
