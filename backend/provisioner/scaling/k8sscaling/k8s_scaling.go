...
type k8sScaling struct {
	...
	clientInit      func(realm string) (*kubernetes.Clientset, error)
...
type ClusterMapper func(realm string) (*kubernetes.Clientset, error)

func NewK8sScaling(clientProvider ClusterMapper, systemNamespace string, disableIstio bool, controllerURL string, instanceName string, mapper NamespaceMapper, cronServiceAccount string, adminServiceAccount string, consoleServiceAccount string, httpServiceAccount string) scaling.RunnerScaling {
	return &k8sScaling{disableIstio: disableIstio,
		...
		cronServiceAccount:        cronServiceAccount,
		adminServiceAccount:       adminServiceAccount,
		httpIngressServiceAccount: httpServiceAccount,
		clientInit:                clientProvider,
		clientLock:                &sync.Mutex{},
		clients:                   map[string]*realmClient{}}
}
...
func (r *k8sScaling) getClient(realm string) (*realmClient, error) {
	r.clientLock.Lock()
	defer r.clientLock.Unlock()
	if ret, ok := r.clients[realm]; ok {
		return ret, nil
	}
	clientset, err := r.clientInit(realm)
	if err != nil {
	...
	if !r.disableIstio {
		...
		if err != nil {
			return nil, errors.Wrap(err, "failed to get server groups")
		}
		...
		for _, group := range groups.Groups {
			if group.Name == "security.istio.io" {
				...
				if err != nil {
					return nil, errors.Wrap(err, "failed to create istio clientset")
				}
				break
			}
		}
	}
	ret := &realmClient{client: clientset, istioSecurity: optional.Ptr(sec)}
	r.clients[realm] = ret
	return ret, nil
...
func (r *k8sScaling) UpdateDeployment(ctx context.Context, deploymentKey string, sch *schema.Module) error {
	...
	rc, err := r.getClient(dk.Payload.Realm)
	if err != nil {
		return errors.Wrap(err, "failed to get kube client")
	}
	deploymentClient := rc.client.AppsV1().Deployments(r.namespaceMapper(sch.Name, dk.Payload.Realm))
	...
	return errors.WithStack(r.handleExistingDeployment(ctx, rc, deployment, sch.Runtime.Scaling.MinReplicas))
}
...
func (r *k8sScaling) StartDeployment(ctx context.Context, deploymentKey string, sch *schema.Module, hasCron bool, hasIngress bool) (url.URL, error) {
	...
	rc, err := r.getClient(dk.Payload.Realm)
	if err != nil {
		return url.URL{}, errors.Wrapf(err, "failed to get kube client")
	}
	logger.Debugf("Creating deployment for %s", deploymentKey)
	namespace, err := r.ensureNamespace(ctx, rc, dk.Payload.Realm, sch)
	if err != nil {
		return url.URL{}, errors.Wrap(err, "failed to ensure namespace")
	}

	deploymentClient := rc.client.AppsV1().Deployments(namespace)
	deployment, err := deploymentClient.Get(ctx, deploymentKey, v1.GetOptions{})
	...
	if deploymentExists {
		...
		err = r.handleExistingDeployment(ctx, rc, deployment, sch.Runtime.Scaling.MinReplicas)
		return r.GetEndpointForDeployment(dk), errors.WithStack(err)

	}
	err = r.handleNewDeployment(ctx, rc, dk.Payload.Realm, module, deploymentKey, sch, hasCron, hasIngress)
	if err != nil {
		return url.URL{}, errors.WithStack(err)
	}
	err = r.waitForDeploymentReady(ctx, rc, namespace, deploymentKey, deployTimeout)
	if err != nil {
...
func (r *k8sScaling) TerminateDeployment(ctx context.Context, deploymentKey string) error {
	...
	cl, err := r.getClient(dk.Payload.Realm)
	if err != nil {
		return errors.Wrap(err, "failed to get kube client")
	}
	serviceClient := cl.client.CoreV1().Services(r.namespaceMapper(dk.Payload.Module, dk.Payload.Realm))
...
func (r *k8sScaling) updateDeployment(ctx context.Context, rc *realmClient, namespace string, name string, mod func(deployment *kubeapps.Deployment)) error {
	deploymentClient := rc.client.AppsV1().Deployments(namespace)
	for range 10 {
...
func (r *k8sScaling) thisContainerImage(ctx context.Context, rc *realmClient) (string, error) {
	deploymentClient := rc.client.AppsV1().Deployments(r.systemNamespace)
...
func (r *k8sScaling) handleNewDeployment(ctx context.Context, rc *realmClient, realm string, module string, name string, sch *schema.Module, cron bool, ingress bool) error {
	logger := log.FromContext(ctx)
	userNamespace := r.namespaceMapper(module, realm)
	cm, err := rc.client.CoreV1().ConfigMaps(r.systemNamespace).Get(ctx, configMapName, v1.GetOptions{})
	if err != nil {
		return errors.Wrapf(err, "failed to get configMap %s", configMapName)
	}
	systemDeploymentClient := rc.client.AppsV1().Deployments(r.systemNamespace)
	userDeploymentClient := rc.client.AppsV1().Deployments(userNamespace)
	...
	servicesClient := rc.client.CoreV1().Services(userNamespace)
	...
	serviceAccountClient := rc.client.CoreV1().ServiceAccounts(userNamespace)
	...
	if sec, ok := rc.istioSecurity.Get(); ok {
		err = r.syncIstioPolicy(ctx, rc, sec, userNamespace, realm, module, name, service, provisionerDeployment, sch, cron, ingress)
		if err != nil {
	...
	thisImage, err := r.thisContainerImage(ctx, rc)
	if err != nil {
...
func (r *k8sScaling) handleExistingDeployment(ctx context.Context, rc *realmClient, deployment *kubeapps.Deployment, replicas int32) error {

	thisContainerImage, err := r.thisContainerImage(ctx, rc)
	if err != nil {
		return errors.WithStack(err)
	}
	...
	if len(changes) > 0 {
		err = r.updateDeployment(ctx, rc, deployment.Namespace, deployment.Name, func(deployment *kubeapps.Deployment) {
			for _, change := range changes {
				change(deployment)
			}
		})
...
func (r *k8sScaling) syncIstioPolicy(ctx context.Context, rc *realmClient, sec istioclient.Clientset, namespace string, realm string, module string, name string, service *kubecore.Service, provisionerDeployment *kubeapps.Deployment, sch *schema.Module, hasCron bool, hasIngress bool) error {
	logger := log.FromContext(ctx)
	...
	for _, callableModule := range callableModuleNames {
		...
		err := r.createOrUpdateIstioPolicy(ctx, sec, callableModuleNamespace, policyName, func(policy *istiosec.AuthorizationPolicy) {
			...
			targetServiceAccount, err := rc.client.CoreV1().ServiceAccounts(callableModuleNamespace).Get(ctx, callableModule, v1.GetOptions{})
			if err != nil {
...
func (r *k8sScaling) waitForDeploymentReady(ctx context.Context, rc *realmClient, namespace string, key string, timeout time.Duration) error {
	logger := log.FromContext(ctx)
	deploymentClient := rc.client.AppsV1().Deployments(namespace)
	podClient := rc.client.CoreV1().Pods(namespace)
...
func (r *k8sScaling) ensureNamespace(ctx context.Context, rc *realmClient, realm string, sch *schema.Module) (string, error) {
	namespace := r.namespaceMapper(sch.Name, realm)
	ns, err := rc.client.CoreV1().Namespaces().Get(ctx, namespace, v1.GetOptions{})
	if err == nil {
		if ns.Labels != nil {
	...
	_, err = rc.client.CoreV1().Namespaces().Create(ctx, ns, v1.CreateOptions{})
	if err != nil {
