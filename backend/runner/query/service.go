...
func (s *Service) Call(ctx context.Context, module *schema.Module, verb *schema.Verb, reqBody []byte) ([]byte, error) {
	...
	jsonRows, _, err := errors.WithStack3(conn.ExecuteQuery(ctx, req))
	if err != nil {
		return nil, err
	}
	...
	case Exec:
		return []byte("{}"), nil
	case One:
		body, err := bodyForType(verb.Response, jsonRows[0])
		if err != nil {
			return nil, errors.WithStack(err)
		}
		return body, nil
...
func bodyForType(typ schema.Type, data []byte) ([]byte, error) {
	switch typ.(type) {
...
func (s *queryConn) ExecuteQuery(ctx context.Context, req ExecuteQueryRequest) (jsonRows []json.RawMessage, rowsAffected int64, err error) {
...
func (s *queryConn) executeQuery(ctx context.Context, db DB, req ExecuteQueryRequest) (jsonRows []json.RawMessage, rowsAffected int64, err error) {
	...
	case One:
		...
		return []json.RawMessage{jsonRow}, 0, nil
...
func scanRowToMap(row any, resultColumns []ResultColumn) (json.RawMessage, error) {
