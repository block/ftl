...
func TestConsumerGroupMembership(t *testing.T) {
	...
	in.Run(t,
		...
		checkGroupMembership("subscriber", "consumeSlow", 1),
		func(t testing.TB, ic in.TestContext) {
			assert.True(t, time.Since(*deploymentKilledTime) < 4*time.Second, "make sure old deployment was removed from consumer group fast enough (%v)", time.Since(*deploymentKilledTime))
		},
