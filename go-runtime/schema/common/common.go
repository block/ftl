...
func ExtractType(pass *analysis.Pass, node ast.Node) optional.Option[schema.Type] {
	maybeType := extractType(pass, node)
	typ, ok := maybeType.Get()
	if !ok {
		return maybeType
	}
	if obj, ok := GetObjectForNode(pass.TypesInfo, node).Get(); ok && obj.Pkg() != nil {
		MarkIncludeNativeName(pass, obj, typ)
	}
	return maybeType
}
...
func extractSelectorType(pass *analysis.Pass, node ast.Node, typ *ast.SelectorExpr) optional.Option[schema.Type] {
	...
	for _, f := range pass.Files {
		if pass.Fset.File(node.Pos()) != pass.Fset.File(f.Pos()) {
			continue
		}
		...
		for _, im := range f.Imports {
			...
			if im.Name != nil && im.Name.Name != ident.Name {
				continue
			}
			if im.Name == nil && getPkgName(path) != ident.Name {
				continue
			}

			switch path + "." + typ.Sel.Name {
			case "time.Time":
			...
			default: // Data ref
				...
				externalModuleName, err := FtlModuleFromGoPackage(path)
				if err != nil {
					return optional.None[schema.Type]()
				}
	...
	return optional.Option[schema.Type]{}
}
...
func extractExternalType(pass *analysis.Pass, node ast.Node) optional.Option[schema.Type] {
	...
	if underlying, ok := obj.Type().(*types.Named); ok &&
		!IsPathInModule(pass.Pkg, tn.Pkg().Path()) && // type is not in this module
